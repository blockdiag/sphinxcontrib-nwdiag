...
class TestSphinxcontribNwdiagErrors(unittest.TestCase):
    @with_app(srcdir='tests/docs/nwdiag', write_docstring=True)
    ...
    @with_app(srcdir='tests/docs/nwdiag', confoverrides=dict(nwdiag_html_image_format='JPG'))
    def test_unknown_format_error(self, app, status, warning):
        app.builder.build_all()
        self.assertIn('unknown format: JPG', warning.getvalue())

    @with_app(srcdir='tests/docs/nwdiag', confoverrides=dict(nwdiag_html_image_format='PDF'))
